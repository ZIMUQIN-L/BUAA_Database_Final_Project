        ...
        <q-card v-bind:style="$q.screen.lt.sm?{'width': '80%'}:{'width':'60%'}">
          ...
          <q-card-section>
            ...
            >
              ...
              <q-input 
                v-model="teacherNewPwd2" 
                filled :type="isPwd2 ? 'password' : 'text'" 
                pattern="[a-zA-Z0-9_]{6,10}"
                hint="请再次确认新密码"
                bottom-slots label="确认用户新密码"
                :rules="[ val => val == teacherNewPwd1 || '请输入与前面一致的密码']">
                <template v-slot:before>
                  <q-icon name="lock"></q-icon>
                </template>
                <template v-slot:append>
    ...
    onClick:function() {
      ...
      axios({
        ...
        params: {
            ...
            "teacherNewPwd": this.teacherNewPwd1
        }
...
          }
    ...
    backLesson:function() {
      this.$router.push('/teacher/lesson' + _this.teacherId)
    }
  }
}
</script>
